...
class ModelManager:
    ...
    def select_model(self, mode: str, domain: Optional[str] = None) -> str:
        if self.ollama_models:
            ...
            if any(k in domain_key for k in ['recon', 'intel', 'osint', 'search']):
                candidates = [m for m in self.ollama_models if any(x in m.lower() for x in ['recon', 'search', 'intel', 'osint', 'mistral', 'qwen'])]
            elif any(k in domain_key for k in ['scan', 'analysis', 'validation', 'audit']):
                candidates = [m for m in self.ollama_models if any(x in m.lower() for x in ['scan', 'analysis', 'audit', 'security', 'sec', 'codellama', 'deepseek'])]
            elif any(k in domain_key for k in ['exploit', 'prioritize', 'report', 'learn', 'code', 'privesc']):
                candidates = [m for m in self.ollama_models if any(x in m.lower() for x in ['code', 'dev', 'exploit', 'gpt', 'infer', 'codellama', 'qwen', 'deepseek'])]
            else:
                candidates = self.ollama_models
            return candidates[0] if candidates else self.ollama_models[0]
...
class OllamaFusion:
    PHASE_MODELS = {
        'recon': ['llama3.2:3b', 'qwen2.5:1.5b', 'codellama:7b', 'phi3:mini', 'gemma2:2b'],
        'vuln': ['codellama:7b', 'deepseek-coder:6.7b', 'qwen2.5-coder:3b', 'llama3.2:3b', 'codegemma:7b', 'starcoder2:3b'],
        'exploit': ['qwen2.5-coder:7b', 'codellama:13b', 'deepseek-coder:6.7b', 'codegemma:7b', 'starcoder2:7b', 'llama3.1:8b', 'phi3:medium'],
        'privesc': ['deepseek-coder:6.7b', 'llama3.2:3b', 'codellama:7b', 'qwen2.5-coder:3b', 'gemma2:9b'],
        'report': ['llama3.1:8b', 'llama3.2:3b', 'gemma2:9b', 'phi3:mini', 'qwen2.5-coder:7b']
    }
    ...
    def _fuse_outputs(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        fused = top_outputs[0] if top_outputs else results[0]['output']
        return {'fused_output': fused, 'confidence': avg_confidence, 'sources': [r['model'] for r in results], 'all_outputs': results}
